...
def __get_post_labels(tags):
    return tags.split(", ")

def __get_post_content(video_code, original, post_credits):
    post_content = video_code + "<br/><br/>" + original + "<br/><br/>" + "Créditos:<br/>" + post_credits
    post_content = post_content.replace("\n", "<br/>")
    return post_content

def __adapt_final_content(content):
    final_content = FinalContent()
    final_content.title = content.title
    final_content.content = __get_post_content(content.video_code, content.original, content.credits)
    final_content.labels = __get_post_labels(content.tags)
...
def get_final_content(editor_content, result_file):
    ...
    final_content = __adapt_final_content(content)
